...
bool transaction(const std::string& filename, bool force, bool reinstall) {
    ...
    provided.reserve(pkg.provides.size());
for (const auto& item : pkg.provides) {
        provided.emplace_back(item->name, item->version);
    }
    for (const auto& [prov_name, prov_version] : provided) {
...
int main(int argc, char** argv) {
    transaction("anemo-epoch.apkg", true, false);
}
